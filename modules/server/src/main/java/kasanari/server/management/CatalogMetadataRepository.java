...
public class CatalogMetadataRepository {
    ...
    public CatalogMetadataRepository(ObjectMapper objectMapper) {
        this.objectMapper = objectMapper;
    }

    public Optional<CatalogMetadata> getById(Handle tx, String catalogId) {
        var query = tx.createQuery(ManagementJdbcQueries.SELECT_CATALOG);
    ...
    public boolean create(Handle tx, CatalogMetadata metadata) {
        ...
        var insert = tx.createUpdate(ManagementJdbcQueries.INSERT_CATALOG);
    ...
    public Optional<CatalogMetadata> update(Handle tx, String catalogId, CatalogSpec spec, Long expectedVersion) {
        ...
        var update = tx.createUpdate(ManagementJdbcQueries.UPDATE_CATALOG);
    ...
    public boolean delete(Handle tx, String catalogId) {
        var delete = tx.createUpdate(ManagementJdbcQueries.DELETE_CATALOG);
    ...
    public void replaceSecrets(Handle tx, String catalogId, Map<String, String> secrets) {
        var delete = tx.createUpdate(ManagementJdbcQueries.DELETE_SECRETS_BY_CATALOG);
        delete.bind(0, catalogId);
        delete.execute();
        ...
        for (var entry : secrets.entrySet()) {
            var upsert = tx.createUpdate(ManagementJdbcQueries.UPSERT_SECRET);
    ...
    public List<String> getSecretKeys(Handle tx, String catalogId) {
        var query = tx.createQuery(ManagementJdbcQueries.SELECT_SECRET_KEYS_BY_CATALOG);
        query.bind(0, catalogId);
