 ...
 *
 * <p>Unlike the Postgres Lance schema (which lacked a tenant column by oversight),
 * every row is scoped by {@code catalog_key} and hash-sharded on that key so all
...
public final class JdbcQueries {
    ...
    public static final String CREATE_NAMESPACES_DDL = """
            CREATE TABLE IF NOT EXISTS kasanari_lance_namespaces (
                ...
                CONSTRAINT kasanari_lance_namespaces_pk PRIMARY KEY (catalog_key HASH, namespace_path ASC)
    ...
    public static final String CREATE_TABLES_DDL = """
            CREATE TABLE IF NOT EXISTS kasanari_lance_tables (
                ...
                CONSTRAINT kasanari_lance_tables_pk PRIMARY KEY (catalog_key HASH, table_id ASC),
                CONSTRAINT kasanari_lance_namespaces_fk FOREIGN KEY (catalog_key, namespace_path)
                    REFERENCES kasanari_lance_namespaces(catalog_key, namespace_path)
            ) WITH (colocation = false)
            """;

    public static final String CREATE_TABLES_FK_INDEX = """
            CREATE INDEX IF NOT EXISTS kasanari_lance_tables_fk_idx
            ON kasanari_lance_tables(catalog_key, namespace_path);
            """;

    public static final String UPSERT_NAMESPACE = """
            INSERT INTO kasanari_lance_namespaces(catalog_key, namespace_path, properties)
            VALUES (:catalog_key, :namespace_path, :properties::jsonb)
            ON CONFLICT (catalog_key, namespace_path)
    ...
    public static final String NAMESPACE_EXISTS = """
            ...
            WHERE catalog_key = :catalog_key AND namespace_path = :namespace_path
            LIMIT 1
            """;
    ...
    public static final String UPSERT_TABLE = """
            INSERT INTO kasanari_lance_tables(catalog_key, table_id, namespace_path, table_name, location, properties)
            VALUES (:catalog_key, :table_id, :namespace_path, :table_name, :location, :properties::jsonb)
            ON CONFLICT (catalog_key, table_id)
            DO UPDATE SET
    ...
    public static final String TABLE_EXISTS = """
            ...
            WHERE catalog_key = :catalog_key AND table_id = :table_id
            LIMIT 1
            """;
